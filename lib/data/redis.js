...
db.prototype = {
	...
	setDbShareCode: function(padId, sharecode,padType) {
		//redisClient.set(REDIS_PREFIX + '-sharecode:' + sharecode ,  padId);
		redisClient.set(REDIS_PREFIX + '-sharecode:' + sharecode ,  padType+'@@'+padId);
	},
	getShareCode: function(sharecode, callback) {
		redisClient.get(REDIS_PREFIX + '-sharecode:' + sharecode , function (err, res) {
			callback(res);
		});
	},
	setDbSectionName: function(padId, sectionname) {
		redisClient.set(REDIS_PREFIX + '-sectionName:' + padId ,  sectionname);		
	},
	getSectionName: function(padId, callback) {
		redisClient.get(REDIS_PREFIX + '-sectionName:' + padId , function (err, res) {
			callback(res);
		});
	},
	setDbTabData: function(padId, tabData) {
		if (tabData !== null) {
			redisClient.hset(REDIS_PREFIX + '-tab:' + padId, padId , tabData);
		}
	},
	getTabData: function(padId, callback) {
		redisClient.hgetall(REDIS_PREFIX + '-tab:' + padId , function (err, res) {			
			//callback(res);
			var texts = [];

			for (var i in res) {
				texts.push( JSON.parse(res[i]) );
